...
(function( $, window, document, undefined ) {
	...
	var List = {
		init: function ( options, elem ) {
			...
			$.each(self.$elem.find('.active[data-action-click=filter]'), function() {
				var val = $.trim($(this).data('value'));

				if( val ){
					self.data.options[ $.trim($(this).data('name')) ] = val;
				}
				
			});

			$.each(self.$elem.find('[data-action-change=filter]'), function() {
				var is = $(this).prop('checked') ? 1:0;

				if( is ){
					self.data.options[ $(this).attr('name') ] = is;
				}
		...
		Events: function () {
			...
			if( self.$header ){
				...
				self.$header.find('[data-action-click=filter]').click(function (e) {
					e.preventDefault();

					self.data.options.pager = 1;
					self.data.options[ $(this).data('name') ] = $(this).data('value');
					$(this).addClass('active').siblings().removeClass('active');

					self.refresh( 1 );
				});

				self.$header.find('[data-action-change=filter]').change(function (e) {
					e.preventDefault();
			...
			self.is_loading = setTimeout(function () {
				self.fetch().done(function( results ) {
					...
					if( self.data.options.sort ){
						...
						sort = sort[0].split(' ');
						self.$tabletitle.find('[data-sort-val=' + sort[0] + ']').attr('data-sort', self.data.options.dir).parent().removeClass( dir=='desc' ? 'asc':'desc' ).addClass( dir );
					}
					// self.buildFrag();
